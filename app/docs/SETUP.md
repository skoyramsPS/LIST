...
2. **Finish the Setup Wizard.** Launch Android Studio. On first run a "Welcome /
   Setup Wizard" appears — accept the **Standard** install and let it download the
   Android SDK + platform-tools. Let it finish before continuing.

3. **Install the command-line tools — this is the step everyone misses.**
   The Setup Wizard installs the SDK Platform but **NOT** the command-line tools,
   and `flutter doctor --android-licenses` runs Android's `sdkmanager`, which lives
   *inside* the command-line tools. Without them you get exactly this error:
   ```
   ...
   ```
   Fix it with the GUI (recommended):
   1. Android Studio → on the Welcome screen click **More Actions ▾ → SDK Manager**
      (or, inside a project: **File → Settings → Languages &amp; Frameworks →
      Android SDK**; on the Welcome screen it's **Customize → All settings… →
      Languages &amp; Frameworks → Android SDK**).
   2. Select the **SDK Tools** tab.
   3. Tick **Android SDK Command-line Tools (latest)**. Also confirm **Android SDK
      Platform-Tools** is ticked.
   4. Click **Apply → OK** and let it download.

   Or do it from a terminal, if `sdkmanager` is already on PATH from a previous
   partial install:
   ```
   sdkmanager "cmdline-tools;latest" "platform-tools"
   ```

4. **Accept the SDK licenses.** Now that cmdline-tools exist, this works:
   ```
   flutter doctor --android-licenses     # press y + Enter for each prompt
   ```
   Then confirm the whole Android row is green:
   ```
   flutter doctor
   ```
   If it *still* says `sdkmanager not found`, your SDK location isn't where Flutter
   looks. Point Flutter at it explicitly (path is shown at the top of SDK Manager,
   usually `%LOCALAPPDATA%\Android\Sdk`):
   ```
   flutter config --android-sdk "%LOCALAPPDATA%\Android\Sdk"
   ```

5. **Get a device — pick ONE of the two:**

   **Option A — Android emulator (no physical phone needed):**
   1. Android Studio → Welcome screen → **More Actions ▾ → Virtual Device Manager**
      (or **Device Manager** inside a project).
   2. Click **Create Device** (the **+**).
   3. Pick a phone, e.g. **Pixel 7** → **Next**.
   4. Choose a system image (e.g. **API 34 / Android 14**); click the **Download**
      link next to it if it isn't installed, wait, then **Next → Finish**.
   5. Press the ▶ **Play** button next to the device to boot the emulator.

   **Option B — physical Android phone:**
   1. On the phone: **Settings → About phone → tap "Build number" 7 times** to
      unlock Developer options.
   2. **Settings → System → Developer options → enable "USB debugging"**.
   3. Connect the phone by USB. On the phone, tap **Allow** on the "Allow USB
      debugging?" prompt.

6. **Confirm the device is visible and run the app:**
   ```
   flutter devices        # your emulator or phone must appear here
   flutter run            # builds + launches TheLIST on Android
   ```

**Other snags.** If `sdkmanager` errors with a Java incompatibility, use Android
Studio's **bundled JDK** rather than a system Java — system Java 17+ is not
compatible with older `sdkmanager`. Either run the licenses command from Android
Studio's built-in terminal, or set `JAVA_HOME` to the JBR that ships with Studio
